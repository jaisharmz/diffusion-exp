...
class Tokenizer:
    ...
    def __call__(self, strings):
        if isinstance(strings, list):
            ...
            self.vocab_size = len(self.vocab.keys())
            return [[self.vocab[ch] for ch in text] for text in strings]
        else:
            for ch in strings:
                if ch not in self.vocab:
                    idx = len(self.vocab)
                    ...
                    self.inv_vocab[idx] = ch
            self.vocab_size = len(self.vocab.keys())
            return [self.vocab[ch] for ch in strings]
    ...
    def decode(self, tokens):
        if isinstance(tokens[0], list):
            return ["".join([self.inv_vocab.get(t, "") for t in string]) for string in tokens]
        else:
